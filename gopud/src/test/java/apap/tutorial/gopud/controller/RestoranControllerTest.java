...
public class RestoranControllerTest {
    ...
    public void whenViewAllRestoranAccessItShouldShowAllRestoranData() throws Exception {
        ...
        mockMvc.perform(get("/restoran/view-all"))
                ...
                .andExpect(model().attribute("restoList", Matchers.hasSize(3)))
                .andExpect(model().attribute("restoList", hasItem(
                        allOf(
                                Matchers.hasProperty("idRestoran", is(1L)),
                                Matchers.hasProperty("nama", is("dummy 1")),
                                Matchers.hasProperty("alamat", is("alamat 1"))
                        )
                )))
                .andExpect(model().attribute("restoList", hasItem(
                        allOf(
                                Matchers.hasProperty("idRestoran", is(2L)),
                                Matchers.hasProperty("nama", is("dummy 2")),
                                Matchers.hasProperty("alamat", is("alamat 2"))
                        )
                )))
                .andExpect(model().attribute("restoList", hasItem(
                        allOf(
                                Matchers.hasProperty("idRestoran", is(3L)),
                                Matchers.hasProperty("nama", is("dummy 3")),
                                Matchers.hasProperty("alamat", is("alamat 3"))
                        )
                )));
